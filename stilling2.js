JS kode · Stillinger som dropdown
Opdateret version med dropdown/accordion. Kopiér hele koden herfra.
"use strict";
...
function updateDropdownStatus(board) {
  const status = board.querySelector(".standings-dropdown-status");

  if (!status) return;

  status.textContent = board.open ? "Luk" : "Åbn";
}

function createBoard(competition, index = 0) {
  const board = document.createElement("details");
  board.className = "standings-board standings-dropdown";
  ...
  summary.className = "standings-board-header standings-summary";

  const titleGroup = document.createElement("div");
  titleGroup.className = "standings-summary-main";
  ...
  status.className = "standings-dropdown-status";
  status.textContent = board.open ? "Luk" : "Åbn";

  const chevron = document.createElement("span");
  chevron.className = "standings-chevron";
  chevron.setAttribute("aria-hidden", "true");

  summary.append(titleGroup, meta, status, chevron);
  ...
  scroll.append(grid);
  board.append(summary, scroll);

  board.addEventListener("toggle", () => {
    if (board.open) {
      document
        .querySelectorAll(".standings-dropdown")
        .forEach((otherBoard) => {
          if (otherBoard !== board) {
            otherBoard.open = false;
            updateDropdownStatus(otherBoard);
          }
        });
    }

    updateDropdownStatus(board);
  });
...
function openBoardFromHash() {
  ...
  const board = target.closest(".standings-dropdown");
...
function renderStandings() {
  ...
  if (standingsCount) {
    standingsCount.textContent = competitions.length;
  }

  openBoardFromHash();
}

window.addEventListener("hashchange", openBoardFromHash);
